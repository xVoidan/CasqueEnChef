...
export const RewardAnimationScreen: React.FC<TrainingStackScreenProps<'RewardAnimation'>> = ({ 
  ...
  const getRarityGlow = (rarity?: string) => {
    switch (rarity) {
      ...
      case 'epic': return '#E91E63';
      case 'rare': return '#00BCD4';
  ...
  const glowAnimatedStyle = useAnimatedStyle(() => ({
    ...
    transform: [
      { scale: interpolate(glowOpacity.value, [0, 1], [0.8, 1.5]) },
    ],
  }));
        ...
        {[...Array(8)].map((_, i) => (
          ...
          >
            <Ionicons 
              name="sparkles" 
              size={24} 
              color={getRarityGlow(currentReward.rarity)} 
            />
          </Animated.View>
        ))}
      </Animated.View>
        ...
        {showingReward && (
          <View style={styles.rewardContainer}>
            ...
            <Animated.View style={[styles.rewardCard, rewardAnimatedStyle]}>
              ...
              >
                <Ionicons 
                  name={getRewardIcon(currentReward) as any} 
                  size={80} 
                  color="#FFFFFF" 
                />
              </LinearGradient>
            ...
            >
              <Text style={[styles.rewardType, { color: colors.textSecondary }]}>
                {currentReward.type === 'badge' ? 'BADGE' : 
                 currentReward.type === 'challenge' ? 'DÉFI COMPLÉTÉ' : 
                 'NOUVEAU RANG'}
              </Text>
              ...
              {currentReward.points !== undefined && currentReward.points !== null && currentReward.points > 0 && (

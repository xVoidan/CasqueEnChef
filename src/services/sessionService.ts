...
class SessionService {
  ...
  async loadQuestions(
    ...
    try {
      ...
      return questions.map(q => {
        ...
        const explicationData = Array.isArray(q.explications) 
          ? q.explications[0] 
          : q.explications;
  ...
  calculateScore(
    ...
    const correctAnswerIds = question.reponses
    ...
    if (question.type_question === 'QCU') {
      ...
      // QCM
      const correctSelections = selectedAnswers.filter(id => 
        correctAnswerIds.includes(id)
      ).length;
      const incorrectSelections = selectedAnswers.filter(id => 
        !correctAnswerIds.includes(id)
      ).length;
    ...
    try {
      ...
      sessionData.questions.forEach((question, index) => {
        ...
        if (sousTheme && sousTheme.theme) {
          ...
          if (answer?.isCorrect) themeStat.correctAnswers++;
          ...
          if (answer?.isCorrect) sousThemeStat.correctAnswers++;
        }
      });
      ...
      const failedQuestions = sessionData.questions
        .map((question, index) => {
          ...
          const sousTheme = sousThemesData?.find(st => st.id === question.sous_theme_id);
          const userAnswerText = answer.selectedAnswers.length > 0
